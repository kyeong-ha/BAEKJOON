...
let number_of_over_height = 0; // 평균 높이보다 높은 땅의 개수
let number_of_under_height = 0; // 평균 높이보다 낮은 땅의 개수
function setRequiredBlocks(){
    for(let i=0; i<ground.length; i++){
        for(let j=0; j<ground[i].length; j++){
            ...
            if(ground[i][j] <= average_of_height)
                number_of_under_height++;
            // else if(ground[i][j] > average_of_height) 
            // number_of_blocks_required--;
            else if(ground[i][j] > average_of_height)
            number_of_over_height++;
        }
    }
}
...
// console.log(number_of_under_height)
// console.log(number_of_over_height)

if(number_of_under_height == 2*number_of_over_height)
    average_of_height++;
else{
